    ...
    <v-card max-width="180px" class="pa-2">
        <v-menu transition="scale-transition" offset-x min-width="auto" absolute>
            <template #activator="{ on }">
                <v-text-field v-model="dateToHTML" label="Date" v-on="on" class="mt-0"></v-text-field>
            </template>
            <v-date-picker v-model="dateToHTML"></v-date-picker>
        </v-menu>
        <v-select :items="getCategoriesList" label="Category" v-model="item.category"></v-select>
        ...
        <v-btn small outlined color="error" class="ml-2" @click="closeForm">Cancel</v-btn>
    </v-card>
</template>
<script>
